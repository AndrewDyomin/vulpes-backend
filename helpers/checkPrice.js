...
async function extractSearchData(body) {
  ...
  const container = $(
    "ol.products.list.items.product-items.motea-list",
  ).first();
...
async function extractHtml(body) {
  ...
  const parse = (t) => Number(t.replace("€", "").replace(",", ".").trim());
...
async function checkPrice() {
  try {
    ...
    while (true) {
      ...
      const batch = await Product.find(query, { article: 1, price: 1, moteaPrice: 1, linkInMotea: 1, _id: 1 })
        .sort({ _id: 1 })
        .limit(BATCH_SIZE)
        .lean();
      ...
      for (const item of batch) {
        ...
        try {
          ...
          if (!mPrice) {
            ...
            const t = data?.find((i) =>
              i.url.includes(item.article.toLowerCase()),
            );
            if (!t) {
          ...
          await Product.findByIdAndUpdate(item._id, {
            moteaPrice: { UAH: mPrice, date: now },
          });
        } catch (err) {
          if (err?.response?.status === 429) {
            console.log("Block 429!!!");
            await Product.findByIdAndUpdate(item._id, {
              moteaPrice: { UAH: item?.moteaPrice?.UAH, date: yesterday },
            });
            await sleep(20000);
          } else if (
